...
static HTTPResponse* http_recv_response(int sock_fd)
{
    ...
    if (body_start) {
        body_start += 4;
        /* IMPORTANT: Do NOT use strlen() here — PNG/JPEG tiles are binary
         * and contain null bytes. Calculate remaining bytes from total_read. */
        resp->body_len = total_read - (size_t)(body_start - buf);
